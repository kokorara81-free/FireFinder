...
SQLite에 데이터가 실제로 쌓였는지 확인하려면 다음 명령을 실행합니다.
...
GitHub Actions에서는 각 작업 시작 전에 GCS의 `firefinder-db/latest/firefinder.db.gz`를 내려받고, 스크리닝 또는 성과분석이 끝난 뒤 `firefinder-db/latest/`만 갱신합니다. DB 파일은 Git에 커밋하지 않습니다. GCS 버킷을 초기화한 첫 실행은 새 DB를 만들고, 이후 실행부터 누적 데이터를 복원합니다.

매주 월요일 UTC 13:00에는 별도 `Weekly FireFinder DB Backup` 워크플로가 전주 DB를 `firefinder-db/weekly/YYYY-Www/firefinder.db.gz`에 보관합니다. `firefinder-db/weekly/latest/` 경로는 사용하지 않습니다. 이 워크플로를 수동 실행하면 실행 시점 기준 전주 ISO 주차에 현재 `latest` DB를 보관합니다.
